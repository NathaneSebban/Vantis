...
export function AuthorizationGate({ target, submitting, onConfirm }: Props) {
  ...
  return (
    <div className="rounded-xl border border-amber-300/70 bg-amber-50 p-4 shadow-[0_10px_30px_-20px_rgba(180,83,9,0.5)]">
      ...
      <p className="mt-2 text-sm leading-relaxed text-amber-900/80">
        ...
        <span className="font-mono font-medium text-amber-800">{target || "this target"}</span>. This is only
      ...
      <label className="mt-4 flex items-start gap-3 text-sm text-amber-900">
        <input
          ...
          className="mt-0.5 h-4 w-4 shrink-0 cursor-pointer accent-violetx"
      ...
      <p id="auth-help" className="mt-2 text-xs text-amber-700/70">
